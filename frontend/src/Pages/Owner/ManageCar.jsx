...
const ManageCar = () => {
  ...
  const deleteProperty = async (propertyId) => {
    try {
      ...
      const { data } = await axios.post('/api/owner/delete-car', { carId: propertyId }); // endpoint name may need change
      if (data.success) {
        toast.success(data.message);
        fetchOwnerProperties();
      } else {
            ...
            {properties.map((property, index) => (
              ...
              >
                ...
                <td className="p-4 max-md:hidden">
                  <span
                    className={`px-2 py-1 text-xs cursor-pointer font-medium rounded-full ${
